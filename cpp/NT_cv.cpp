...
// [[Rcpp::export]]
NumericVector NewtonMC_CV(NumericMatrix Trail, NumericVector Zij, NumericMatrix CVij, double t, double h1, double h2, int n, int nn, int p) {
    ...
    MatrixXd CVijC = MatrixXd::Map(CVij.begin(), n, n);
    ...
    for (int z = 0; z < nn; z++) {
        if (TrailC(z, 2) > bw1 && TrailC(z, 2) < bw2) {
            ...
            NijT1(i - 1, j - 1) += inv_sqrt_2pi / h1 * std::exp(-0.5 * diff * diff) * CVijC(i - 1, j - 1);
        }
        if (TrailC(z, 2) > bw3 && TrailC(z, 2) < bw4) {
            i = TrailC(z, 0);
            j = TrailC(z, 1);
            diff = (TrailC(z, 2) - t) / h2;
            NijT2(i - 1, j - 1) += inv_sqrt_2pi / h2 * std::exp(-0.5 * diff * diff) * CVijC(i - 1, j - 1);
        }
    }
    ...
    while (conv > 0.0001) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Eij(i, n - 1) = exp(temp) * CVijC(i, n - 1);
        }
