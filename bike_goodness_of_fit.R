...
sourceCpp(file = "NT_ini.cpp", verbose = TRUE, rebuild = TRUE)
sourceCpp(file = "CI.cpp", verbose = TRUE, rebuild = TRUE)
sourceCpp(file = "CI_alt.cpp", verbose = TRUE, rebuild = TRUE)
...
zij <- bike_data$zij[1:100,1:100,c(3,4)]
trail <- bike_data$trail %>% filter(x <= 100, y<= 100)
...
for (pk in 1:(2 * n + p - 1)) {
  a_b_g_hat[[pk]] <- local({
    ...
    function(s) {
      fit <- smooth.spline(seq(0.1,0.9,0.01), xkk[current_pk, ])
